...
def calculate_year(year: int):
    try:
        return f'Суммарные затраты за {year} год составили: {storage[str(year)]["sum"]}<b> ₽'
    except KeyError:
...
def calculate_month(year: int, month: int):
    try:
        return f'Суммарные затраты за {month} - {year} составили: <b>{storage[str(year)][str(month)]}<b/> ₽'
    except KeyError:
